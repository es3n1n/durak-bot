...
class Client(SocketListener):
    def __init__(
        ...
        self.logger = getLogger('durakonline')
    ...
    def update_avatar(self, file: str) -> None:
        self.send_server({'command': 'update_avatar', 'base64': file})
    ...
    def buy_prem(self, id: int = 0) -> None:
        self.send_server({'command': 'buy_prem', 'id': f'com.rstgames.durak.prem.{id}'})

    def buy_points(self, id: int = 0) -> dict:
        self.send_server({'command': 'buy_points', 'id': f'com.rstgames.durak.points.{id}'})

        return self.listen()

    def buy_asset(self, asset_id: str) -> None:
        self.send_server({'command': 'buy_asset', 'id': asset_id})
    ...
    def send_user_message_code(self, code: str, content: str) -> None:
        self.send_server({'command': 'send_user_msg_code', 'code': code, 'msg': content})

    def gift_coll_item(self, item_id: int, coll_id: str, to: int) -> dict:
        self.send_server(
            {
    ...
    def get_server(self) -> Server:
        self.send_server({'command': 'get_server'})
        return Server(self._get_data('server').get('id'))

    def update_name(self, nickname: str | None = None) -> None:
        self.send_server({'command': 'update_name', 'value': nickname})

    def save_note(self, note: str, user_id: int, color: int = 0) -> None:
        self.send_server({'command': 'save_note', 'note': note, 'color': color, 'id': user_id})

    def leaderboard_get_by_user(self, user_id: int, type: str = 'score', season: bool = False) -> objects.Leaderboard:
        s = '' if not season else 's_'
        self.send_server({'command': s + 'lb_get_by_user', 'user_id': user_id, 'type': type})
    ...
    def leaderboard_get_top(self, type: str = 'score', season: bool = False) -> objects.Leaderboard:
        s = '' if not season else 's_'
        self.send_server({'command': f'{s}lb_get_top', 'type': type})
        response = self._get_data('lb')
        return decode(json.dumps(response), type=objects.Leaderboard)

    def leaderboard_get_by_place_down(
        self, place: int = 20, type: str = 'score', season: bool = False
    ) -> objects.Leaderboard:
        s = '' if not season else 's_'
        self.send_server({'command': f'{s}lb_get_by_place_down', 'place': place, 'type': type})
        response = self._get_data('lb')
        return decode(json.dumps(response), type=objects.Leaderboard)
